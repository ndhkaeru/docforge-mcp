...
const REPO = (process.env.GITHUB_REPOSITORY || 'ndhkaeru/docloupe-mcp').split('/')[1] || 'docloupe-mcp';
...
function requestJson(url) {
  return new Promise((resolve, reject) => {
    const headers = {
      'User-Agent': 'docloupe-mcp-release-packager',
...
function download(url, outputPath) {
  return new Promise((resolve, reject) => {
    const headers = { 'User-Agent': 'docloupe-mcp-release-packager' };
    if (process.env.GITHUB_TOKEN) headers.Authorization = `Bearer ${process.env.GITHUB_TOKEN}`;
    https.get(url, { headers }, (response) => {
...
async function main() {
  ...
  for (const [npmPlatform, releasePlatform, suffix] of TARGETS) {
    ...
    for (const server of SERVERS) {
      ...
      const assetName = `docloupe-mcp-${server}-tools-${releasePlatform}${suffix}`;
