...
Download the latest binaries for your platform from the [Releases page](https://github.com/ndhkaeru/docloupe-mcp/releases). Supported platform keys are `windows-x64`, `linux-x64`, `macos-x64`, and `macos-arm64`.

Each server is published as a separate executable:

| Binary | Use it for |
| --- | --- |
| `docloupe-mcp-md-tools-<platform>[.exe]` | Structure-aware Markdown reads and edits. |
| `docloupe-mcp-excel-tools-<platform>[.exe]` | Excel-family Markdown export plus `.xlsx` workbook inspection and editing. |
| `docloupe-mcp-pdf-tools-<platform>[.exe]` | PDF-to-Markdown text extraction. |
| `docloupe-mcp-docx-tools-<platform>[.exe]` | DOCX-to-Markdown conversion. |
| `docloupe-mcp-pptx-tools-<platform>[.exe]` | PPTX-to-Markdown conversion. |
| `docloupe-mcp-csv-tools-<platform>[.exe]` | CSV-to-Markdown tables. |
| `docloupe-mcp-html-tools-<platform>[.exe]` | HTML-to-Markdown conversion. |
| `docloupe-mcp-text-tools-<platform>[.exe]` | Plain text and Markdown passthrough reads. |
| `docloupe-mcp-json-tools-<platform>[.exe]` | JSON/JSONL passthrough reads. |
...
```powershell
codex mcp add docloupe-md -- C:\path\to\docloupe-mcp-md-tools-windows-x64.exe
codex mcp add docloupe-excel -- C:\path\to\docloupe-mcp-excel-tools-windows-x64.exe
```

**Claude Code**

```powershell
claude mcp add docloupe-md -- C:\path\to\docloupe-mcp-md-tools-windows-x64.exe
```

**Gemini CLI**

```powershell
gemini mcp add docloupe-md -- C:\path\to\docloupe-mcp-md-tools-windows-x64.exe
```
...
Agents waste context and become brittle when they rely on raw file dumps or one-off conversion scripts. DocLoupe MCP gives them bounded, format-aware tools:

| Without it | With `docloupe-mcp` |
| --- | --- |
...
command = "/opt/docloupe/docloupe-mcp-md-tools-linux-x64"
args = []
enabled = true

[mcp_servers.excel-tools]
command = "/opt/docloupe/docloupe-mcp-excel-tools-linux-x64"
args = []
enabled = true
```
</details>
...
```json
{
  "mcpServers": {
    "docloupe-md": {
      "command": "C:\\tools\\docloupe-mcp-md-tools-windows-x64.exe",
      "args": []
    },
    "docloupe-excel": {
      "command": "C:\\tools\\docloupe-mcp-excel-tools-windows-x64.exe",
      "args": []
    }
  }
}
